                ...
                <div className="hidden lg:block">
                    <div ref={viewportRef} className="relative bg-white rounded-2xl mb-16 p-8 md:p-12 overflow-hidden min-h-[400px] shadow-lg">
                        <motion.div
                            className="flex z-10"
                            drag="x"
                            dragConstraints={{ left: -Math.max(0, (testimonials.length - 1) * (slideWidth + 40)), right: 0 }}
                            dragElastic={0.2}
                            onDragEnd={(_, info) => {
                                if (info.offset.x < -50 && index < testimonials.length - 1) setIndex(index + 1)
                                else if (info.offset.x > 50 && index > 0) setIndex(index - 1)
                            }}
                            animate={{ x: -index * (slideWidth + 40) }}
                            style={{ cursor: 'grab' }}
                        ...
                        >
                            {testimonials.map((t, i) => (
                                <div
                                    ...
                                    style={{
                                        transform: i === 0 ? 'translateX(0)' : i === 1 ? 'translateX(100px)' : i === 2 ? 'translateX(250px)' : i === 3 ? 'translateX(360px)' : 'translateX(500px)',
                                    }}
                                >
